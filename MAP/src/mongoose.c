...
static void mg_dns_free(struct mg_connection *c, struct dns_data *d) {
  LIST_DELETE(struct dns_data, (struct dns_data **)&c->mgr->active_dns_requests,
              d);
  free(d);
}
...
static bool ff_remove(const char *path) { return f_unlink(path) == FR_OK; }

static bool ff_mkdir(const char *path) { return f_mkdir(path) == FR_OK; }
...
static void p_close(void *fp) { fclose((FILE *)fp); }
...
static bool p_remove(const char *path) { return remove(path) == 0; }

static bool p_mkdir(const char *path) { return mkdir(path, 0775) == 0; }
...
static void p_close(void *fp) { (void)fp; }
...
bool mg_json_get_num(struct mg_str json, const char *path, double *v) {
  ...
  if ((n = mg_json_get(json.ptr, (int)json.len, path, &toklen)) >= 0 &&
    ...
    if (v != NULL)
      *v = mg_atod(json.ptr + n, toklen, NULL);
    found = 1;
  }
  return found;
}
...
void mg_log_set_fn(void (*fn)(unsigned char)) { s_log_func = fn; }

static void logc(unsigned char c) { s_log_func(c); }
...
void mg_mqtt_login(struct mg_connection *c, const struct mg_mqtt_opts *opts) {
  ...
  if (c->is_mqtt5)
    total_len += 1U + (hdr[7] & MQTT_HAS_WILL ? 1U : 0);
  ...
  if (c->is_mqtt5)
    mg_send(c, &zero, sizeof(zero)); // V5 properties
  mg_send_u16(c, mg_htons((uint16_t)cid.len));
  mg_send(c, cid.ptr, cid.len);

  if (hdr[7] & MQTT_HAS_WILL) {
    if (c->is_mqtt5)
      mg_send(c, &zero, sizeof(zero)); // will props
...
void mg_rpc_process(void **head, struct mg_str s, mg_pfn_t pfn, void *pfnd) {
  ...
  if (off > 0 && s.ptr[off] == '"') {
    ...
    while (h != NULL && !mg_match(m, h->method, NULL))
      h = h->next;
    if (h != NULL) {
...
bool mg_open_listener(struct mg_connection *c, const char *url) {
  ...
  if (success == false && fd != INVALID_SOCKET)
    closesocket(fd);
  return success;
}
...
static int mg_net_send(void *ctx, const unsigned char *buf, size_t len) {
  ...
  if (n < 0) {
    if (mg_sock_would_block())
      return MBEDTLS_ERR_SSL_WANT_WRITE;
    if (mg_sock_conn_reset())
...
static size_t mip_driver_stm32_tx(const void *buf, size_t len, void *userdata) {
  ...
  if (sr & BIT(2))
    ETH->DMASR = BIT(2), ETH->DMATPDR = 0; // Resume
  if (sr & BIT(5))
    ETH->DMASR = BIT(5), ETH->DMATPDR = 0; // if busy
  if (len == 0)
    printf("E: D0 %lx SR %lx\n", (long)s_txdesc[0][0], (long)sr);
  return len;
  (void)userdata;
}
...
void ETH_IRQHandler(void) {
  ...
  if (sr & BIT(6)) { // Frame received, loop
    for (uint32_t i = 0; i < ETH_DESC_CNT; i++) {
      if (s_rxdesc[i][0] & BIT(31))
        continue;
      uint32_t len = ((s_rxdesc[i][0] >> 16) & (BIT(14) - 1));
      //    printf("%lx %lu %lx %lx\n", i, len, s_rxdesc[i][0], sr);
      if (s_rx != NULL)
        s_rx(s_rxbuf[i], len > 4 ? len - 4 : len, s_rxdata);
      s_rxdesc[i][0] = BIT(31);
    }
  }
  if (sr & BIT(7))
...
static size_t q_avail(struct queue *q) {
  size_t n = 0;
  if (q->tail != q->head)
    q_copyout(q, (uint8_t *)&n, sizeof(n), q->tail);
  return n;
}
...
static void arp_cache_init(uint8_t *p, int n, int size) {
  for (int i = 0; i < n; i++)
    p[2 + i * size] = (uint8_t)(2 + (i - 1) * size);
  for (int i = 0; i < n; i++)
    p[3 + i * size] = (uint8_t)(2 + (i + 1) * size);
...
static uint8_t *arp_cache_find(struct mip_if *ifp, uint32_t ip) {
  ...
  if (ip == 0)
    return NULL;
  if (p[0] == 0 || p[1] == 0)
    arp_cache_init(p, MIP_ARP_ENTRIES, 12);
...
static struct ip *tx_ip(struct mip_if *ifp, uint8_t proto, uint32_t ip_src,
  ...
  if (!mac)
    mac = arp_cache_find(ifp, ifp->gw); // No, use gateway
  if (mac)
    memcpy(eth->dst, mac, sizeof(eth->dst)); // Found? Use it
  if (!mac)
    memset(eth->dst, 255, sizeof(eth->dst));    // No? Use broadcast
...
void mg_connect_resolved(struct mg_connection *c) {
  ...
  if (ifp->eport < MIP_ETHEMERAL_PORT)
    ifp->eport = MIP_ETHEMERAL_PORT;
  if (c->is_udp) {
    c->loc.ip = ifp->ip;
